...
// Цикл завершается по SIGTERM: тикер останавливается, а начатый проход
// досчитывается — за него держит inflight в processDueNotifications.
func (b *Bot) RunScheduler() {
	...
	for range ticker.C {
		if b.stopping.Load() {
			log.Println("планировщик уведомлений остановлен")
			return
		}
		b.processDueNotifications()
	}
...
func (b *Bot) processDueNotifications() {
	// Проход отмечается как незавершённая работа: уведомление, которое уже
	// помечено sent_at в БД, обязано долететь до клиента, иначе оно потеряется
	// навсегда — повторно эта строка не выбирается.
	if !b.trackStart() {
		return
	}
	defer b.trackDone()

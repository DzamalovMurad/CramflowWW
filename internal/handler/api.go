...
func (a *API) Routes() http.Handler {
	...
	api.HandleFunc("POST /api/launch", a.trackLaunch)
	api.HandleFunc("GET /api/subscription-bonus", a.getSubscriptionBonus)
	api.HandleFunc("POST /api/subscription-bonus", a.claimSubscriptionBonus)
...
func (a *API) createOrder(w http.ResponseWriter, r *http.Request) {
	...
	in.TelegramID = tgID
	// Метка канала из подписанной initData важнее пришедшей в теле: подпись
	// проверена ботом, тело — нет. Значение из тела остаётся только для входа
	// с сайта по ?src=, где initData нет вовсе.
	if src := model.SourceFromStartParam(startParam); src != "" {
		in.Source = src
	}
